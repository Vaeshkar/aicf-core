...
exports.AIActivityMonitor = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const os = __importStar(require("os"));
const fs = __importStar(require("fs/promises"));
const events_1 = require("events");
class AIActivityMonitor extends events_1.EventEmitter {
    constructor() {
        super();
        this.watchers = [];
    ...
    async start() {
        ...
        const platformsToMonitor = config.get('platformsToMonitor', ['claude', 'copilot', 'augment']);
    ...
    async setupPlatformMonitoring(platform) {
        const patterns = this.PLATFORM_PATTERNS[platform];
        if (!patterns) {
    ...
    async createWatcher(platform, watchPath) {
    ...
    handleFileEvent(event, platform, uri) {
    ...
    isConversationFile(filePath) {
        ...
        const hasConversationKeyword = conversationIndicators.some(keyword => filename.includes(keyword));
        const hasSupportedExtension = supportedExtensions.includes(extension);
        return hasConversationKeyword && hasSupportedExtension;
    }
    startIdleDetection() {
        const config = vscode.workspace.getConfiguration('aicf');
        const idleTimeout = config.get('idleTimeout', 300) * 1000; // Convert to ms
        this.activityTimer = setInterval(() => {
            this.checkForIdleConversations();
        }, 60000); // Check every minute
    }
    checkForIdleConversations() {
        const config = vscode.workspace.getConfiguration('aicf');
        const idleTimeout = config.get('idleTimeout', 300) * 1000;
        const now = new Date();
    ...
    async expandPathWithWildcards(pathPattern) {
    ...
    async triggerDiscovery() {
        ...
        const discoveries = [];
        const config = vscode.workspace.getConfiguration('aicf');
        const platforms = config.get('platformsToMonitor', ['claude', 'copilot', 'augment']);
    ...
    async findConversationFiles(dir) {
        try {
            const files = [];
...
exports.AIActivityMonitor = AIActivityMonitor;
//# sourceMappingURL=AIActivityMonitor.js.map